...
public class TheQueue<Type> implements TheQueueInterface<Type> {
    ...
    public Type dequeue() {//Chatgpt
        return _queue.poll();
    }

    public Type getFront() {//Chatpgt
        return _queue.peek();
    }
    ...
    public String[] getCodons() {//Gemini
        int size = _queue.size();
        String[] codons = new String[size / 3];
        for (int i = 0; i < codons.length; i++) {
            String codon = (String) _queue.poll() + _queue.poll() + _queue.poll();
            codons[i] = codon;
        }
        return codons;
    }
